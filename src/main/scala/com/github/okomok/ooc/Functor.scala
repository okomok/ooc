...
/*
    ...
    def bihom(C: Category): Functor = new Functor {
        override val dom: Category = _Product_(C.op, C)
        override def cod: Category = _Set_
    ...
    def opposite(F: Functor): Functor = new Functor {
        override val dom: Category = F.dom.op
        override val cod: Category = F.cod.op
